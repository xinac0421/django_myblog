...
def tools_math_plugin(request):
    if request.method == 'POST':
        ...
        if math_form.is_valid():
            ...
            all_math_format_list = [math_format_list[x:x + columns_num] for x in
                                    range(0, len(math_format_list), columns_num)]
            context = {'all_math_list': all_math_list, 'all_math_format_list': all_math_format_list}
